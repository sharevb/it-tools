...
export function parse9F10_IAD(hex: string): string[] {
  ...
  if (bytes.length === 22 || bytes.length === 24) {
    results.push('Likely Visa profile');
  } else if (bytes.length === 32 || bytes.length === 34) {
    results.push('Likely Mastercard profile');
  } else {
    results.push('Scheme unknown');
  }
...
export function parse9F33_TerminalCapabilities(hex: string): string[] {
  const bytes = hexToBytes(hex);
  if (bytes.length !== 3) {
    throw new Error('9F33 must be 3 bytes');
  }
  const results: string[] = [];

  if (hasBit(bytes[0], 7)) {
    results.push('Cash transaction supported');
  }
  if (hasBit(bytes[0], 6)) {
    results.push('Goods purchase supported');
  }
  if (hasBit(bytes[0], 5)) {
    results.push('Services purchase supported');
  }
  if (hasBit(bytes[0], 4)) {
    results.push('Cashback supported');
  }

  if (hasBit(bytes[1], 7)) {
    results.push('Plaintext PIN for ICC supported');
  }
  if (hasBit(bytes[1], 6)) {
    results.push('Enciphered PIN (online) supported');
  }
  if (hasBit(bytes[1], 5)) {
    results.push('Signature supported');
  }
  if (hasBit(bytes[1], 4)) {
    results.push('No CVM supported');
  }

  if (hasBit(bytes[2], 7)) {
    results.push('Terminal risk management supported');
  }
  if (hasBit(bytes[2], 6)) {
    results.push('Issuer authentication supported');
  }
  if (hasBit(bytes[2], 5)) {
    results.push('CDA supported');
  }
...
export function parse9F34_CVMResults(hex: string): string[] {
  const bytes = hexToBytes(hex);
  if (bytes.length !== 3) {
    throw new Error('9F34 must be 3 bytes');
  }
  const results: string[] = [];
  ...
  if (hasBit(bytes[2], 7)) {
    results.push('CVM performed');
  }
  if (hasBit(bytes[2], 6)) {
    results.push('CVM failed');
  }
  if (hasBit(bytes[2], 5)) {
    results.push('Bypass attested');
  }
...
export function parse9F40_AdditionalTerminalCapabilities(hex: string): string[] {
  const bytes = hexToBytes(hex);
  if (bytes.length < 4) {
    throw new Error('9F40 must be at least 4 bytes');
  }
  const results: string[] = [];

  if (hasBit(bytes[0], 7)) {
    results.push('Cash supported');
  }
  if (hasBit(bytes[0], 6)) {
    results.push('Cashback supported');
  }
  if (hasBit(bytes[0], 5)) {
    results.push('Purchase with cashback supported');
  }
  if (hasBit(bytes[0], 4)) {
    results.push('Refund supported');
  }

  if (hasBit(bytes[1], 7)) {
    results.push('Offline plaintext PIN supported');
  }
  if (hasBit(bytes[1], 6)) {
    results.push('Offline enciphered PIN supported');
  }
  if (hasBit(bytes[1], 5)) {
    results.push('Online PIN supported');
  }
  if (hasBit(bytes[1], 4)) {
    results.push('Signature supported');
  }
  ...
  if (hasBit(bytes[2], 7)) {
    results.push('SDA supported');
  }
  if (hasBit(bytes[2], 6)) {
    results.push('DDA supported');
  }
  if (hasBit(bytes[2], 5)) {
    results.push('CDA supported');
  }
  if (hasBit(bytes[2], 4)) {
    results.push('Combined data authentication supported');
  }
...
export function parse9F66_TTQ(hex: string): string[] {
  const bytes = hexToBytes(hex);
  if (bytes.length < 4) {
    throw new Error('9F66 must be at least 4 bytes');
  }
  const results: string[] = [];

  if (hasBit(bytes[0], 7)) {
    results.push('Contactless EMV mode supported');
  }
  if (hasBit(bytes[0], 6)) {
    results.push('Contactless MSD mode supported');
  }
  if (hasBit(bytes[0], 5)) {
    results.push('Contact EMV supported');
  }
  if (hasBit(bytes[0], 4)) {
    results.push('Offline data authentication supported');
  }

  if (hasBit(bytes[1], 7)) {
    results.push('No signature (QPS) supported');
  }
  if (hasBit(bytes[1], 6)) {
    results.push('CVM required');
  }
  if (hasBit(bytes[1], 5)) {
    results.push('Online PIN supported');
  }
  ...
  if (hasBit(bytes[2], 7)) {
    results.push('Online cryptogram required');
  }
  if (hasBit(bytes[2], 6)) {
    results.push('Issuer update processing supported');
  }
...
export function parse9F6C_CTQ(hex: string): string[] {
  const bytes = hexToBytes(hex);
  if (bytes.length < 2) {
    throw new Error('9F6C must be at least 2 bytes');
  }
  const results: string[] = [];
  ...
  if (hasBit(bytes[1], 7)) {
    results.push('Offline approval possible');
  }
  if (hasBit(bytes[1], 6)) {
    results.push('Online processing possible');
  }
  if (hasBit(bytes[1], 5)) {
    results.push('CVM performed by card');
  }
...
export function parse9F6E_FFI(hex: string): string[] {
  const bytes = hexToBytes(hex);
  if (bytes.length < 1) {
    throw new Error('9F6E must be at least 1 byte');
  }
  const results: string[] = [];
...
export function parseEmvData(tlvData: string, kernel: string) {
  ...
  const describeItem = (item: EmvTag): EmvTag => {
    ...
    const getActiveBitsDescriptions = (bits: EmvBitDescription[][]) =>
      bits
        .flat()
        .filter((b) => b.description !== 'RFU')
        .map((b) => `${b.description}: ${b.value === '1' ? 'yes' : 'no'}`);
