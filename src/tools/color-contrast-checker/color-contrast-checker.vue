...
<template>
  <div max-w-600px>
    <n-form-item :label="t('tools.color-contrast-checker.texts.label-forecolor')" label-placement="left" mb-1>
      <n-color-picker v-model:value="forecolor" :modes="['hex']" :show-alpha="false" />
    </n-form-item>
    <n-form-item :label="t('tools.color-contrast-checker.texts.label-backcolor')" label-placement="left" mb-1>
      <n-color-picker v-model:value="backcolor" :modes="['hex']" :show-alpha="false" />
    </n-form-item>
    ...
    <c-card v-if="!result.error" :title="t('tools.color-contrast-checker.texts.title-wcag-2-1')">
      <input-copyable
        readonly
        :label="t('tools.color-contrast-checker.texts.label-wcag-constrast')"
        label-width="130px"
        label-position="left"
        :value="result.wcag21ConstrastValue ?? ''"
        mb-3
      />

      <n-form-item
        v-if="result.level"
        :label="t('tools.color-contrast-checker.texts.label-contrast-quality')"
        label-width="130px"
        label-placement="left"
      >
        <n-tag
          :max="5"
          :color="{ color: result.level.bg, textColor: result.level.fg, borderColor: result.level.fg }"
          mr-2
        >
      ...
      <n-form-item
        v-if="result.smallLevel"
        :label="t('tools.color-contrast-checker.texts.label-small-texts')"
        label-width="130px"
        label-placement="left"
      >
        <n-tag
          :color="{ color: result.smallLevel.bg, textColor: result.smallLevel.fg, borderColor: result.smallLevel.fg }"
        >
          <n-rate :count="3" readonly :default-value="result.smallLevel.stars" :color="result.smallLevel.fg" />
        </n-tag>
      </n-form-item>
      <n-form-item
        v-if="result.largeLevel"
        :label="t('tools.color-contrast-checker.texts.label-large-texts')"
        label-width="130px"
        label-placement="left"
      >
        <n-tag
          :color="{ color: result.largeLevel.bg, textColor: result.largeLevel.fg, borderColor: result.largeLevel.fg }"
        >
      ...
      <c-card :title="t('tools.color-contrast-checker.texts.title-normal-text')">
        <n-form-item
          :label="t('tools.color-contrast-checker.texts.label-wcag-2-1-aa')"
          label-width="130px"
          label-placement="left"
        >
        ...
        <n-form-item
          :label="t('tools.color-contrast-checker.texts.label-wcag-2-1-aaa')"
          label-width="130px"
          label-placement="left"
        >
      ...
      <c-card :title="t('tools.color-contrast-checker.texts.title-large-text')">
        <n-p
          ><i>{{
            t(
              'tools.color-contrast-checker.texts.tag-text-that-is-14pt-18-67px-and-bold-or-any-text-bigger-than-18pt-24px-is-defined-as-large-scale-text',
            )
          }}</i></n-p
        >
        <n-form-item
    ...
    <c-card v-if="!result.error" :title="t('tools.color-contrast-checker.texts.title-apca')">
      <input-copyable
        readonly
        :label="t('tools.color-contrast-checker.texts.label-apca-constrast')"
        label-width="130px"
        label-position="left"
        :value="result.apcaContrastValue ?? ''"
        mb-2
      />
    </c-card>
  </div>
