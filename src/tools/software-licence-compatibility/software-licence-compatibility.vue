...
const conditions = [
  ...
  {
    key: 'include-copyright--source',
    desc: 'A copy of the license and copyright notice must be included with the software in source form, but is not required for binaries.',
  },
  ...
  {
    key: 'same-license--file',
    desc: 'Modifications of existing files must be released under the same license when distributing the software. In some cases a similar or related license may be used.',
  },
  {
    key: 'same-license--library',
    desc: 'Modifications must be released under the same license when distributing the software. In some cases a similar or related license may be used, or this condition may not apply to works that use the software as a library.',
  },
];
...
const limitations = [
  ...
  {
    key: 'patent-use',
    desc: 'This license explicitly states that it does NOT grant any rights in the patents of contributors.',
  },
...
const filteredLicencesInfos = computed(() =>
  licenses
    .filter((licence) => {
      return (
        includesAll(filterAllowedPermissions.value, licence.permissions) &&
...
function handleLicenceFallback() {
  const fallback =
    filteredLicencesInfos.value && filteredLicencesInfos.value.length > 0
      ? filteredLicencesInfos.value[0]
      : { value: '', label: '' };
...
<template>
  <div>
    <n-card :title="t('tools.software-licence-compatibility.texts.title-software-licences-compatibility')" mb-5>
      <n-form-item
        :label="t('tools.software-licence-compatibility.texts.label-first-license-spdx')"
      ...
      <n-form-item
        :label="t('tools.software-licence-compatibility.texts.label-second-license-spdx')"
        label-placement="left"
        label-width="120"
      >
        <n-auto-complete
          ...
          :options="licenceInfos"
          :placeholder="
            t('tools.software-licence-compatibility.texts.placeholder-put-your-second-licence-spdx-licence')
          "
          raw-text
          mb-5
        />
      ...
      <div v-if="firstLicence !== '' && secondLicence !== ''" flex justify-center>
        <span v-if="compatibility === 'ok'">
          <n-icon color="green"> <CheckIcon /> </n-icon
          >{{ t('tools.software-licence-compatibility.texts.tag-compatible') }}</span
        >
        <span v-if="compatibility === 'nok'">
          <n-icon color="red"> <CrossIcon /> </n-icon
          >{{ t('tools.software-licence-compatibility.texts.tag-not-compatible') }}</span
        >
    ...
    <n-card :label="t('tools.software-licence-compatibility.texts.label-licence-infos')">
      ...
      <n-p text-center> Applicable licences: {{ filteredLicencesInfos.length }} </n-p>
      ...
      <div v-if="filteredLicencesInfos.length > 0">
        ...
        <n-form-item :label="t('tools.software-licence-compatibility.texts.label-content')">
          <TextareaCopyable :value="currentLicenceInfo?.body ?? ''" word-wrap readonly mb-5 />
        </n-form-item>
      </div>
    </n-card>
  </div>
