...
const isbnAuditInfos = computed<{ isValid: boolean; clues: CKeyValueListItems }>(() => {
  ...
  return {
    isValid,
    clues: Array.from(isbn?.clues ?? [], (clue) => ({
      label: `${clue.message} (${clue.groupname})`,
      value: clue.candidate,
    })),
  };
});
</script>
...
<template>
  <div>
    <c-input-text
      v-model:value="rawIsbn"
      :placeholder="t('tools.isbn-validator-and-parser.texts.placeholder-enter-an-isbn-to-check-for-validity')"
      test-id="isbn-input"
    />
    <n-alert v-if="!isbnAuditInfos.isValid" type="error">
      {{ t('tools.isbn-validator-and-parser.texts.tag-invalid-isbn')
      }}<input-copyable
        v-if="normalizedISBN"
        :label="t('tools.isbn-validator-and-parser.texts.label-probably-correct')"
        label-position="left"
    ...
    <c-card
      v-if="isbnAuditInfos.clues.length > 0"
      mt-5
      :title="t('tools.isbn-validator-and-parser.texts.title-isbn-audit-infos')"
    >
      <c-key-value-list :items="isbnAuditInfos.clues" data-test-id="isbn-info" />
    </c-card>
  </div>
</template>
