**`chmod`** (_change mode_) sets who may read, write, and execute a file or directory on Unix-like systems. The mode can be written **symbolically** (`u+x`) or **numerically** (`755`) — both reach the same result.
...
Three digits map to **user, group, others**. An optional fourth _leading_ digit sets the special bits (`chmod 4755`, see below).
...
`x` on a directory means _traverse_, not _execute_ — without it nothing below the directory can be reached, no matter how permissive the files themselves are.
