...
describe('ip-include-exclude', () => {
  describe('substractCIDRs', () => {
    it('should return error on invalid values', () => {
      expect(substractCIDRs({ allowedRanges: '192.168.0.', disallowedRanges: '' })).to.deep.eq({
        // NOSONAR
        allowedCIDRs: [],
        allowedSubnets: [],
        disallowedSubnets: [],
    ...
    it('should return correct substractions and subnets', () => {
      expect(substractCIDRs({ allowedRanges: '192.168.3.0/24', disallowedRanges: '' })).to.deep.eq({
        // NOSONAR
      ...
      expect(substractCIDRs({ allowedRanges: '192.168.2.0/24', disallowedRanges: '192.168.2.1' })).to.deep.eq({
        // NOSONAR
        allowedCIDRs: [
          '192.168.2.0/32', // NOSONAR
      ...
      expect(
        substractCIDRs({
          allowedRanges: '192.168.12.0/24',
          disallowedRanges: '192.168.12.1-192.168.12.10, 192.168.12.34',
        }),
      ).to.deep.eq({
        // NOSONAR
        allowedCIDRs: [
          '192.168.12.0/32', // NOSONAR
