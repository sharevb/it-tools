  ...
  switch (bitCount) {
    ...
    case 64: {
      ...
      return [...uint]
        .slice(0, 2)
        .reverse()
        .map((p) => p.toString(2).padStart(32, '0'))
        .join('');
    }
    default:
      throw new Error(
        t('tools.floating-point-number-converter.model.text.unsupported-bit-count-only-32-and-64-are-allowed'),
      );
  }
}
...
export function convertBinaryToDecimal({
  ...
  if (decimalPrecision.match(/[^\d]/)) {
    throw new Error(
      t('tools.floating-point-number-converter.model.text.decimal-precision-must-be-a-positive-whole-number'),
    );
  }
  ...
  switch (value.length) {
    ...
    default:
      throw new Error(
        t(
          'tools.floating-point-number-converter.model.text.invalid-length-supply-a-binary-string-with-length-32-or-64',
        ),
      );
  }
  ...
  let resultString =
    decimalPrecision.length === 0 ? result.toString() : result.toFixed(Number.parseInt(decimalPrecision));
...
export function calcErrorDueToConversion({ decimalInput, actualValue }: { decimalInput: string; actualValue: string }) {
  // toFixed(32) is valid since ES2018 (0-100) and needed for sub-1e-20 conversion errors
