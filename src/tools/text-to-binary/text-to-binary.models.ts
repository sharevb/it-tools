...
function convertTextToUtf8Binary(
  text: string,
  { separator = ' ', base = 2 }: { separator?: string; base?: EncodingBase } = {},
): string {
  if (!text?.trim()) {
    return '';
  }
...
function convertUtf8BinaryToText(binary: string, { base = 2 }: { base?: EncodingBase } = {}): string {
  ...
  return new TextDecoder(undefined, { fatal: true }).decode(Uint8Array.from(codepoints));
}
