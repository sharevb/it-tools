  ...
  <n-card :title="t('tools.cookies-parser.texts.title-cookie-parser')">
    <n-space vertical>
      <n-input
        ...
        :placeholder="t('tools.cookies-parser.texts.placeholder-enter-cookie-header')"
        type="text"
      />
    </n-space>
    <n-p>
      {{ t('tools.cookies-parser.texts.tag-tip-navigate-to-a-website-to-test-open-f12-console-and-run')
      }}<n-tag :bordered="false">
        <code>{{ t('tools.cookies-parser.texts.tag-window-copy-document-cookie') }}</code> </n-tag
      >{{
        t(
          'tools.cookies-parser.texts.tag-this-will-copy-the-cookie-string-to-your-clipboard-note-this-will-only-copy-cookies-available-to-javascript-http-only-and-secure-cookies-will-be-omitted',
        )
      }}
    </n-p>
    <n-table v-if="parsedCookies.length">
      <thead>
        <tr>
          <th>{{ t('tools.cookies-parser.texts.tag-name') }}</th>
          <th>{{ t('tools.cookies-parser.texts.tag-value') }}</th>
        </tr>
      </thead>
      <tbody>
