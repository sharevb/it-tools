...
const { copy } = useCopy({
  source: pickedLines,
  text: t('tools.random-line-picker.texts.text-picked-lines-copied-to-clipboard'),
});

const maxCount = ref<number | undefined>(undefined);

watch(
  [() => config.value.repeat, () => config.value.input],
  () => {
...
<template>
  <div>
    <c-card>
      <c-input-text
        v-model:value="config.input"
        test-id="input"
        multiline
        :placeholder="t('tools.random-line-picker.texts.placeholder-put-your-text-here')"
        rows="10"
      />
      ...
      <n-space mt-5>
        ...
        <n-form-item
          :label="t('tools.random-line-picker.texts.label-prefix-lines-with-line-number')"
