...
const hashTypes = ['sha1', 'sha256', 'sha384', 'sha512', 'md5'];
...
const [signOutput, signError] = computedCatch(
  () => {
    const privateKey = sshpk.parsePrivateKey(signPrivateKey.value, 'auto', {
      passphrase: signPrivateKeyPassphrase.value,
    });
...
<template>
  <div>
    <c-card :title="t('tools.rsa-ecdsa-signing.texts.title-sign')">
      <div>
        ...
        <div flex flex-1 flex-col gap-2>
          ...
          <c-input-text
            ...
            :label="t('tools.rsa-ecdsa-signing.texts.label-your-private-key-password')"
            clearable
            raw-text
          />
        </div>
      ...
      <c-alert
        v-if="signError && signPrivateKey !== ''"
        type="error"
        mt-12
        :title="t('tools.rsa-ecdsa-signing.texts.title-error-while-signing')"
      >
        {{ signError }}
      </c-alert>
    ...
    <c-card :title="t('tools.rsa-ecdsa-signing.texts.title-verify')">
      ...
      <c-alert
        v-if="verifyError && verifyPublicKey !== ''"
        type="error"
        mt-12
        :title="t('tools.rsa-ecdsa-signing.texts.title-error-while-verifying')"
      >
        {{ verifyError }}
      </c-alert>
      <c-alert
        v-if="verifyOutput && verifyPublicKey !== ''"
        type="error"
        mt-12
        :title="t('tools.rsa-ecdsa-signing.texts.title-signature-failed')"
      >
      ...
      <n-alert
        ...
        type="success"
        mt-12
        :title="t('tools.rsa-ecdsa-signing.texts.title-signature-verified')"
      >
