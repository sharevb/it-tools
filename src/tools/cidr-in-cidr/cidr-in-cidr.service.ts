...
export function cidrInCidr({ baseRange, ipOrRangeToTest }: { baseRange: string; ipOrRangeToTest: string }) {
  ...
  const baseSubnets = baseMatchMasks
    .map((mask) => {
  ...
  return {
    baseSubnets,
    isIncluded: baseMatchMasks.some((baseMask) =>
      ipOrRangeToTestMatchMasks.every((ipOrRangeMask) => ipOrRangeMask.isSubsetOf(baseMask)),
    ),
  };
}
