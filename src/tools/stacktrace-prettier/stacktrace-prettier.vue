...
const stackType = useQueryParamOrStorage<'net' | 'js' | 'java' | 'python'>({
  name: 'type',
  storageName: 'stackfmt:type',
  defaultValue: 'net',
});
...
const { copy: copyText } = useCopy({
  source: stackTraceText,
  text: t('tools.stacktrace-prettier.texts.text-formatted-stacktrace-copied-to-the-clipboard'),
});
const { copy: copyMarkdown } = useCopy({
  source: stackTraceMarkdown,
  text: t('tools.stacktrace-prettier.texts.text-markdown-formatted-stacktrace-copied-to-the-clipboard'),
});

async function downloadAsPNG() {
  const dataUrl = await domtoimage.toPng(formatedStackTrace.value, {
    bgcolor: styleStore.isDarkTheme ? '#333' : '#fff',
  });
...
<template>
  <div>
    <n-radio-group v-model:value="stackType" name="radiogroup" mb-2 flex justify-center>
      <n-space>
        <n-radio value="net" :label="t('tools.stacktrace-prettier.texts.label-net')" />
        <n-radio value="js" :label="t('tools.stacktrace-prettier.texts.label-javascript')" />
        <n-radio value="python" :label="t('tools.stacktrace-prettier.texts.label-python')" />
        <n-radio value="java" :label="t('tools.stacktrace-prettier.texts.label-java')" />
      </n-space>
    </n-radio-group>
    ...
    <pre
      ref="formatedStackTrace"
      class="stacktrace"
      :style="{ padding: '20px', whiteSpace: wrap ? 'pre-wrap' : 'pre', overflowX: 'auto', wordBreak: 'break-all' }"
    />
