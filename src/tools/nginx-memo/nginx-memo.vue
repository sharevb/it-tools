...
const memoImports = import.meta.glob('./nginx-memo.content*.md');
const memoComponent = ref<Component | null>(null);

async function loadMemo(currentLocale = locale.value) {
  const memoKey = `./nginx-memo.content.${currentLocale}.md`;
  const loader = memoImports[memoKey] ?? memoImports['./nginx-memo.content.md'];
