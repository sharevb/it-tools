...
function generateSQL() {
  const { sql, error } = generateSQLFromTemplate(sqlTemplate.value, parameters.value);
    ...
    <NForm label-placement="top">
      <NFormItem :label="t('tools.sql-parameters.texts.label-parameterized-sql-statement')">
        <NInput
          ...
          type="textarea"
          :placeholder="
            t('tools.sql-parameters.texts.placeholder-e-g-select-from-users-where-name-name-and-age-and-type-type')
          "
          :autosize="{ minRows: 3 }"
        />
      </NFormItem>
      ...
      <n-card :title="t('tools.sql-parameters.texts.title-parameters')">
        <n-p>{{
          t(
            'tools.sql-parameters.texts.tag-for-positional-parameters-leave-name-empty-for-named-parameters-enter-name-without-or-prefix',
          )
        }}</n-p>
        <NDynamicInput
