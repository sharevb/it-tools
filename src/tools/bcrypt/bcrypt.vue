  ...
  <c-card :title="t('tools.bcrypt.texts.title-hash')">
    ...
    <n-form-item :label="t('tools.bcrypt.texts.label-salt-count')" label-placement="left" label-width="120">
      <n-input-number
        ...
        :placeholder="t('tools.bcrypt.texts.placeholder-salt-rounds')"
        :max="20"
        :min="4"
        w-full
      />
    ...
    <div mt-1 h-3 op-60>
      {{
        hashState.timeTakenMs == null
          ? ''
          : t('tools.bcrypt.texts.hashed-in-elapsed-period', {
              elapsedPeriod: new Intl.DurationFormat(locale, { style: 'long' }).format({
                milliseconds: hashState.timeTakenMs,
              }),
            })
      }}
    </div>
    ...
    <n-form label-width="120">
      <n-form-item :label="t('tools.bcrypt.texts.label-your-string')" label-placement="left">
        <c-input-text
          ...
          :placeholder="t('tools.bcrypt.texts.placeholder-your-string-to-compare')"
          raw-text
        />
      </n-form-item>
      <n-form-item :label="t('tools.bcrypt.texts.label-your-hash')" label-placement="left">
        <c-input-text
          ...
          :placeholder="t('tools.bcrypt.texts.placeholder-your-hash-to-compare')"
          raw-text
        />
      </n-form-item>
        ...
        <c-input-text
          ...
          :value="
            compareState.result == null
              ? undefined
              : compareState.result
                ? t('tools.bcrypt.texts.matched')
                : t('tools.bcrypt.texts.no-match')
          "
      ...
      <div mb-1 mt-1 h-3 op-60>
        {{
          compareState.timeTakenMs == null
            ? ''
            : t('tools.bcrypt.texts.compared-in-elapsed-period', {
                elapsedPeriod: new Intl.DurationFormat(locale, { style: 'long' }).format({
                  milliseconds: compareState.timeTakenMs,
                }),
              })
        }}
      </div>
