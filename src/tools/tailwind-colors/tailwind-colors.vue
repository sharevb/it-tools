...
import { type TailwindColor, informationalSections, tailwindColors } from './tailwind-colors.service';
...
const selectedFormat = ref<'tailwind' | 'hex' | 'rgb' | 'hsl' | 'oklch'>('tailwind');
...
async function copyColor(colorValue: string) {
  try {
    ...
    message.error('Failed to copy color to clipboard. Please try again.');
  }
}
</script>
      ...
      >
        ...
        <div v-else-if="section.type === 'faq'" class="faq-container">
          <div
            ...
            class="faq-item"
          >
            <h3 v-if="typeof faq !== 'string'">
              {{ faq.question }}
            </h3>
            <p v-if="typeof faq !== 'string'">
              {{ faq.answer }}
            </p>
          </div>
        </div>
