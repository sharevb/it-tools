...
const memoImports = import.meta.glob('./git-sem.memo*.md');
const memoComponent = ref<Component | null>(null);

async function loadMemo(currentLocale = locale.value) {
  const memoKey = `./git-sem.memo.${currentLocale}.md`;
  const loader = memoImports[memoKey] ?? memoImports['./git-sem.memo.md'];
