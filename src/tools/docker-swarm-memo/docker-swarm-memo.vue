...
const memoImports = import.meta.glob('./docker-swarm*.md');
const memoComponent = ref<Component | null>(null);

async function loadMemo(currentLocale = locale.value) {
  const memoKey = `./docker-swarm.${currentLocale}.md`;
  const loader = memoImports[memoKey] ?? memoImports['./docker-swarm.md'];
