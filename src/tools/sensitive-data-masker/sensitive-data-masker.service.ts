...
const urlWithOrWithoutPrefixRegex =
  /\b(https?:\/\/)?(www\.)?[a-zA-Z0-9@:%._+~#=-]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_+.~#?&\/=]*)\b/g;

export type MatcherNames =
  | 'uuid'
  | 'creditCard'
  | 'ssn'
  | 'url'
...
export function maskSensitiveData({
  ...
  return maskString(
    ...
    null as never,
    {
      ...
      urlWithOrWithoutPrefixRegex: excludedMatchers.includes('urlWithOrWithoutPrefix')
        ? emptyRegex
