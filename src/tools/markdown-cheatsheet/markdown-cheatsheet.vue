...
const memoImports = import.meta.glob('./md-memo.content*.md');
const memoComponent = ref<Component | null>(null);

async function loadMemo(currentLocale = locale.value) {
  const memoKey = `./md-memo.content.${currentLocale}.md`;
  const loader = memoImports[memoKey] ?? memoImports['./md-memo.content.md'];
