...
<template>
  <div style="flex: 0 0 100%">
    <div mx-auto max-w-600px>
      <c-file-upload
        :title="t('tools.image-exif-reader.texts.title-drag-and-drop-a-image-file-here-or-click-to-select-a-file')"
        @file-upload="onImageUploaded"
      />
      ...
      <div v-if="status === 'error'">
        <c-alert mt-4> Error parsing image file: {{ errorMessage }} </c-alert>
      </div>
      ...
      <c-card
        v-if="status === 'parsed' && openStreetMapUrl"
        :title="t('tools.image-exif-reader.texts.title-gps-infos')"
        mt-4
      >
        <div flex gap-2>
      ...
      <div font-size-3>
        {{ t('tools.image-exif-reader.texts.tag-made-with')
        }}<a href="https://github.com/mattiasw/ExifReader" target="_blank">{{
          t('tools.image-exif-reader.texts.tag-exifreader')
        }}</a>
      </div>
    </div>
  </div>
