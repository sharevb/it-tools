...
const memoImports = import.meta.glob('./zellij-memo.content*.md');
const memoComponent = ref<Component | null>(null);

async function loadMemo(currentLocale = locale.value) {
  const memoKey = `./zellij-memo.content.${currentLocale}.md`;
  const loader = memoImports[memoKey] ?? memoImports['./zellij-memo.content.md'];
