...
function parseJSON(value: string) {
  try {
    ...
    return JSON.parseBigNum(
      value.replace(
        /`((?:[^`]|\\`)*)`/g,
        (_m, s) => `"${s.replace(/"/g, '\\""').replace(/\r/g, '\\r').replace(/\n/g, '\\n')}"`,
      ),
    );
  }
}
...
const transformer = (value: string) =>
  withDefaultOnError(() => JSON.stringify(parseJSON(value), null, indentSize.value), '');
...
<template>
  <div style="flex: 0 0 100%">
    <div style="margin: 0 auto" flex justify-center gap-3>
      <n-form-item
        :label="t('tools.javascript-to-json.texts.label-indent-size')"
