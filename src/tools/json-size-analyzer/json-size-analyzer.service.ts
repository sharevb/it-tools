...
function getTreeNodes(obj: AnalysisNode, parentName: string): TreeNode {
  ...
  const biggest_child_node = obj.__meta__.biggest_node_child
    ? ` ; biggest child node: '${obj.__meta__.biggest_node_child}'`
    : '';
  const parent_relative_percentage = obj.__meta__.parent_relative_percentage
    ? ` ; ${obj.__meta__.parent_relative_percentage} of parent`
    : '';
  return {
    key: parentName,
    label: obj.__meta__
