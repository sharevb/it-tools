...
describe('decodeTransaction', () => {
  it('decodes a valid transaction', () => {
    ...
    const data = iface.encodeFunctionData('transfer', ['0x000000000000000000000000000000000000dead', 1000n]);
  ...
  it('decode raw transaction', () => {
    ...
    expect(decodeTransaction(abi, rawTransaction)).toEqual({
      ...
      outputJson: {
        ...
        method: 'registerOffChainDonation',
        names: ['addr', 'timestamp', 'chfCents', 'currency', 'memo'],
        types: ['address', 'uint256', 'uint256', 'string', 'bytes32'],
      },
  ...
  it('throws if data does not start with 0x', () => {
    ...
    expect(() => decodeTransaction(transferAbi, badTx)).toThrowError('Transaction data must start with 0x');
  });
});
